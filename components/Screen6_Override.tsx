...
const Screen6_Override: React.FC<Props> = ({ onComplete }) => {
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto bg-white shadow-lg md:rounded-xl overflow-hidden border border-gray-100">
      {/* Header */}
      <div className="p-4 border-b bg-white flex items-center gap-3">
        ...
        <div>
          <h3 className="font-bold text-gray-800">Salmane</h3>
          <span className="text-xs text-green-500">Online</span>
        </div>
      </div>

      {/* Messages */}
      <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-pink-50/30">
        {messages.map((msg, i) => (
          <motion.div
            key={i}
          ...
          >
             ...
             <div className="bg-white p-4 rounded-2xl rounded-bl-none shadow-sm border border-pink-100 max-w-[85%]">
      ...
      {showButton && (
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="p-6 bg-white border-t flex justify-center"
        >
          <button 
            onClick={onComplete}
            className="bg-pink-500 hover:bg-pink-600 text-white px-8 py-3 rounded-full font-bold shadow-lg flex items-center gap-2 transition-all hover:scale-105"
          >
            I have the code <ArrowRight className="w-5 h-5" />
          </button>
        </motion.div>
      )}
    </div>
